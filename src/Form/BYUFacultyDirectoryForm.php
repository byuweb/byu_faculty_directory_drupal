    ...
    private function getFacultyFromParent(bool $filter, string $departments){
        ...
        foreach ($result['data'] as $entry) {
            ...
            if ($node = reset($uid_query)){
                ...
                if (!$node->field_awards_override->value){
                    $node->field_awards = $entry['field_awards'][0]['value'];
                }

                if (!$node->field_biography_override->value){
                    $node->field_biography = $entry['field_biography'][0]['value'];
                }

                if (!$node->field_committees_override->value){
                    $node->field_committees = $entry['field_committees'][0]['value'];
                }

                if (!$node->field_courses_override->value){
                    $node->field_courses = $entry['field_courses'][0]['value'];
                }

                if (!$node->field_email_override->value){
                    $node->field_email = $entry['field_email'][0]['value'];
                }

                if (!$node->field_first_name_override->value){
                    $node->field_first_name = $entry['field_first_name'][0]['value'];
                }

                if (!$node->field_last_name_override->value){
                    $node->field_last_name = $entry['field_last_name'][0]['value'];
                }

                if (!$node->field_links_override->value){
                    $node->field_links = $entry['field_links'][0]['value'];
                }

                if (!$node->field_office_location_override->value){
                    $node->field_office_location = $entry['office_location'][0]['value'];
                }

                if (!$node->field_phone_number_override->value){
                    $node->field_phone_number = $entry['field_phone_number'][0]['value'];
                }

                if (!$node->field_publications_override->value){
                    $node->field_publications = $entry['field_publications'][0]['value'];
                }

                if (!$node->field_research_long_override->value){
                    $node->field_research_long = $entry['field_research_long'][0]['value'];
                }

                if (!$node->field_education_override->value){
                    $node->field_education = $entry['field_education'][0]['value'];
                }

                if (!$node->field_title_override->value){
                    $node->field_title = $entry['field_title'][0]['value'];
                }
                ...
                $create_array['type'] = 'byu_faculty_member';
                $create_array['langcode'] = 'en';
                $create_array['field_awards'] = $entry['field_awards'][0]['value'];
                $create_array['field_biography'] = $entry['field_biography'][0]['value'];
                $create_array['field_committees'] = $entry['field_committees'][0]['value'];
                $create_array['field_courses'] = $entry['field_courses'][0]['value'];
                $create_array['field_email'] = $entry['field_email'][0]['value'];
                $create_array['field_first_name'] = $entry['field_first_name'][0]['value'];
                $create_array['field_last_name'] = $entry['field_last_name'][0]['value'];
                $create_array['field_links'] = $entry['field_links'][0]['value'];
                $create_array['field_office_location'] = $entry['office_location'][0]['value'];
                $create_array['field_phone_number'] = $entry['field_phone_number'][0]['value'];
                $create_array['field_publications'] = $entry['field_publications'][0]['value'];
                $create_array['field_research_long'] = $entry['field_research_long'][0]['value'];
                $create_array['field_education'] = $entry['field_education'][0]['value'];
                $create_array['field_title'] = $entry['field_title'][0]['value'];
